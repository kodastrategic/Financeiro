...
if(window.Chart){
  ...
  if(Chart.defaults.scales){const _sc=Chart.defaults.scales.linear;if(_sc&&_sc.grid)_sc.grid.color='rgba(255,255,255,0.06)';if(_sc&&_sc.ticks)_sc.ticks.color='#6e6e73';if(_sc&&_sc.border)_sc.border.display=false;}
...
async function renderChartBalanceLine(tx){
  ...
  if(!dates.length){makeChart('chartBalanceLine',{type:'line',data:{labels:['Sem dados'],datasets:[{data:[0],borderColor:'rgba(255,255,255,0.12)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
...
function renderChartExpenseCategory(tx){
  ...
  if(!labels.length){makeChart('chartExpenseCategory',{type:'doughnut',data:{labels:['Sem dados'],datasets:[{data:[1],backgroundColor:['rgba(255,255,255,0.06)'],borderWidth:0}]},options:doughnutOptions()});attachPieLegend('chartExpenseCategory',[]);return;}
  makeChart('chartExpenseCategory',{type:'doughnut',data:{labels,datasets:[{data,backgroundColor:colors,borderWidth:2,borderColor:'#1c1c1e'}]},options:doughnutOptions()});
...
function renderChartIncomeCategory(tx){
  ...
  labels=sorted.map(x=>x.l);data=sorted.map(x=>x.v);
  const colors=labels.map(l=>catColor(l));
  if(!labels.length){makeChart('chartIncomeCategory',{type:'doughnut',data:{labels:['Sem dados'],datasets:[{data:[1],backgroundColor:['rgba(255,255,255,0.06)'],borderWidth:0}]},options:doughnutOptions()});attachPieLegend('chartIncomeCategory',[]);return;}
  makeChart('chartIncomeCategory',{type:'doughnut',data:{labels,datasets:[{data,backgroundColor:colors,borderWidth:2,borderColor:'#1c1c1e'}]},options:doughnutOptions()});
...
function renderChartMonthlyExpense(tx){
  if(dashboardFilter!=='all'){
    ...
    if(total===0){makeChart('chartMonthlyExpense',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
  ...
  if(!months.length){makeChart('chartMonthlyExpense',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
...
function renderChartIncomePeriod(tx){
  if(dashboardFilter!=='all'){
    ...
    if(total===0){makeChart('chartIncomePeriod',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
  ...
  if(!labels.length){makeChart('chartIncomePeriod',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
...
function renderChartExpensePeriod(tx){
  if(dashboardFilter!=='all'){
    const target=dashboardFilter;
    const total=filterTxByMonth(tx,target).filter(t=>t.type==='expense').reduce((s,t)=>s+t.amount,0);
    if(total===0){makeChart('chartExpensePeriod',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
  ...
  const labels=Object.keys(periods).sort().slice(-12),values=labels.map(l=>periods[l]);
  if(!labels.length){makeChart('chartExpensePeriod',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
...
function renderChartInvestmentLine(tx){
  ...
  const dates=Object.keys(daily).sort(),vals=dates.map(d=>daily[d]);
  if(!dates.length){makeChart('chartInvestmentLine',{type:'line',data:{labels:['Sem dados'],datasets:[{data:[0],borderColor:'rgba(255,255,255,0.12)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
...
function renderChartComparison(tx){
  if(dashboardFilter!=='all'){
    ...
    if(!incomes&&!expenses){makeChart('chartComparison',{type:'bar',data:{labels:['Sem dados'],datasets:[{label:'Receitas',data:[0],backgroundColor:'rgba(255,255,255,0.06)'},{label:'Despesas',data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
  ...
  if(!months.length){makeChart('chartComparison',{type:'bar',data:{labels:['Sem dados'],datasets:[{label:'Receitas',data:[0],backgroundColor:'rgba(255,255,255,0.06)'},{label:'Despesas',data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,plugins:{legend:{display:false}}}});return;}
...
function renderChartTopExpense(tx){
  ...
  if(!labels.length){makeChart('chartTopExpense',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,indexAxis:'y',plugins:{legend:{display:false}}}});return;}
...
function renderChartTopIncome(tx){
  ...
  const sorted=Object.entries(groups).sort((a,b)=>b[1]-a[1]);
  const labels=sorted.slice(0,8).map(e=>e[0]),values=sorted.slice(0,8).map(e=>e[1]),colors=labels.map(l=>catColor(l));
  if(!labels.length){makeChart('chartTopIncome',{type:'bar',data:{labels:['Sem dados'],datasets:[{data:[0],backgroundColor:'rgba(255,255,255,0.06)'}]},options:{responsive:true,indexAxis:'y',plugins:{legend:{display:false}}}});return;}
