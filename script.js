...
function setupChat(){
  ...
  $('#fileInput').addEventListener('change',(e)=>{const f=e.target.files[0];if(f)handleImageFile(f);e.target.value='';});
  $('#cameraInput').addEventListener('change',(e)=>{const f=e.target.files[0];if(f)handleImageFile(f);e.target.value='';});
  ...
  // OCR preview buttons
  $('#ocrSend').addEventListener('click',sendOcrText);
  $('#ocrCancel').addEventListener('click',cancelOcr);
}
...
async function runOcr(dataUrl){
  try{
    const {data}=await Tesseract.recognize(dataUrl,'por',{logger:()=>{}});
...
function sendOcrText(){
  const text=$('#ocrText').value.trim();
  if(!text)return;
  cancelOcr();
  addChatMessage(`<span>📄 ${escapeHtml(text)}</span>`,'msg-ocr');
  // Tenta interpretar como comando, senão cola no input
  if(text.match(/^\/(\w+)\s+([\d.,]+)$/)){
    processCommand(text);
  }else{
